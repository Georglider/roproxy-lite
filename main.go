...
func requestHandler(ctx *fasthttp.RequestCtx) {
	...
	_, disableProxyKey := os.LookupEnv("DISABLE_PROXYKEY")
	...
	if ok && string(ctx.Request.Header.Peek("PROXYKEY")) != val {
		...
		ctx.Request.Header.Set("x-api-key", val)
	}
	...
	response := makeRequest(ctx, val, 1)
...
func makeRequest(ctx *fasthttp.RequestCtx, key string, attempt int) *fasthttp.Response {
	if attempt > retries {
	...
	if err != nil {
		...
		return makeRequest(ctx, key, attempt+1)
	}
